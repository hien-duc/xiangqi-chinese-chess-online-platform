...
import { AnimCurrent } from "@/components/ui/board/_lib/anim.ts";
import { DragCurrent } from "@/components/ui/pieces/_lib/drag.ts";
import { Drawable } from "@/components/ui/board/_lib/draw.ts";
import { timer } from "@/utils/util.ts";
...
export interface HeadlessState {
  ...
  movable: {
    ...
    color?: cg.Color | "both"; // color that can move. red | black | both
  ...
  predroppable: {
    ...
    current?: {
      // current saved predrop {role: 'horse'; key: 'e4'}
      role: cg.Role;
      key: cg.Key;
    };
    events: {
  ...
  draggable: {
    ...
    autoDistance: boolean; // lets xiangqiground set distance to zero when user drags pieces
