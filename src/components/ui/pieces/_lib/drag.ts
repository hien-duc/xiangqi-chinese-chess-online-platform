...
import { clear as drawClear } from "@/components/ui/board/_lib/draw.ts";
import * as cg from "@/utils/types.ts";
import { anim } from "@/components/ui/board/_lib/anim.ts";
...
export function start(s: State, e: cg.MouchEvent): void {
  ...
  if (piece && element && stillSelected && board.isDraggable(s, orig)) {
    ...
    if (ghost) {
      ...
      util.translate(
        ghost,
        util.posToTranslate(bounds)(util.key2pos(orig), board.redPov(s))
      );
  ...
  requestAnimationFrame(() => {
    ...
    if (!origPiece || !util.samePiece(origPiece, cur.piece)) cancel(s);
      ...
      if (cur.started) {
        ...
        cur.keyHasChanged ||=
          cur.orig !== board.getKeyAtDomPos(cur.pos, board.redPov(s), bounds);
      }
    }
    processDrag(s);
  });
